...
const ProductivityLeaderBoard = {
    ProductivityChart: asyncHandler(async (req, res) => {
        try {
            ...
            const employees = await Employee.find({ company_name: company?.Company_Id });
            const timesheetData = await TimeSheet.find({ company: company?.Company_Id });
            ...
            const result = await Promise.all(employees.map(async (employee) => {
                ...
                const total_employee_working_days = employeeTimesheets.reduce((acc, sheet) => acc + sheet.hours, 0);
                ...
                const ok_hours = employeeTimesheets.reduce((acc, sheet) => acc + (sheet.ok_hours || 0), 0);
                const blank_hours = employeeTimesheets.reduce((acc, sheet) => acc + (sheet.blank_hours || 0), 0);
               
                const productivity = total_hours > 0 ? Math.round((billed_hours / total_hours) * 100 * 100) / 100 : 0;
